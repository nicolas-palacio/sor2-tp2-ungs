...
//NODO: 2 (TCP) INTERFAZ: 0;
static void
TraceCwnd2 (std::string cwnd_tr_file_name)
{
  AsciiTraceHelper ascii;
  cWndStream = ascii.CreateFileStream (cwnd_tr_file_name.c_str ());
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow", MakeCallback (&CwndTracer)); // /3 o 2/.../0
}

//NODO: 2 (TCP) INTERFAZ: 0;
static void
TraceSsThresh2 (std::string ssthresh_tr_file_name)
{
  AsciiTraceHelper ascii;
  ssThreshStream = ascii.CreateFileStream (ssthresh_tr_file_name.c_str ());
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/0/SlowStartThreshold", MakeCallback (&SsThreshTracer));///3 o 2/.../0
}
...
int main (int argc, char *argv[])
{
  ...
  std::string cwnd_name_node_2="tcp-2-0";
  uint16_t port=400;
  bool tracing=true;
  ...
  //Genero las conexiones punto a punto
  ...
  //Se instala la pila de protocolos TCP/IPv4
...
//-------------------------------------------
  ...
  if(tracing){
     ...
     Ptr<OutputStreamWrapper> ascii_wrap;
      ...
      Simulator::Schedule (Seconds (2.000001), &TraceCwnd2,cwnd_name_node_2 + "-cwnd.data");
      Simulator::Schedule (Seconds (2.000001), &TraceSsThresh2, cwnd_name_node_2 + "-ssth.data");    
  }
